 ...
 * -----
 * Last Modified:  2021-05-24  1:06:34
...
function divideBySign(num: string, sign: string): string {
  let reg = /\d{1,3}(?=(\d{3})+$)/g;
...
export function formatByParam(raw: string, formatString: string):string{
  let result: string = ''
...
export function format(param: string):string {
...
export function injectFormatToString(): void {
  // @ts-ignore
